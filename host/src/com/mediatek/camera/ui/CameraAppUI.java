...
public class CameraAppUI implements IAppUi {
    ...
    //add start 2019、09、24 add beautyFace module
    private BeautyFaceManager mBeautyFaceManager;
    ...
    public void onCreate() {
        ...
        //start 2019、09、24 add beautyFace module
        mBeautyFaceManager = new BeautyFaceManager(mApp, parentView);
        mBeautyFaceManager.setBeautyFaceClickedListener(new OnmPhotoManagerClickedListenerImpl());
        mViewManagers.add(mBeautyFaceManager);
        //end
     ...
     *
     * @param bitmap the bitmap matched with the picture or video, such as
    ...
    public void updateCurrentMode(final String modeEntry) {
        ...
        if (mModeProvider != null) {
            ...
            LogHelper.e(TAG, "111111!!----mCurrentMode====" + item.mMode);
    ...
    public void onCameraSelected(final String cameraId) {
        ...
        if (!isMainThread()) {
            ...
            if ("0".equals(mCurrentCameraId)) {
                ...
                msg.arg1 = IAppUi.BEAUTY_FACE;
                msg.arg2 = View.GONE;
                msg.what = SET_UI_VISIBILITY;
                mConfigUIHandler.sendMessage(msg);
                LogHelper.d(TAG, "111111!!607--onCameraSelected z=" + ++z);
            } else {
                Message msg = Message.obtain();
                msg.arg1 = IAppUi.BEAUTY_FACE;
                ...
                msg.what = SET_UI_VISIBILITY;
                mConfigUIHandler.sendMessage(msg);
                LogHelper.d(TAG, "111111!!615--onCameraSelected y=" + ++y);
            }
        } else {
            int i = 0;
            int k = 0;
            if ("0".equals(mCurrentCameraId)) {
                setUIVisibilityImmediately(IAppUi.BEAUTY_FACE, View.GONE);
                LogHelper.d(TAG, "111111!!622--onCameraSelected i=" + ++i);
            } else {
                setUIVisibilityImmediately(IAppUi.BEAUTY_FACE, View.VISIBLE);
                LogHelper.d(TAG, "111111!!625--onCameraSelected j=" + ++k);
            }
        }
    ...
    private void configUIVisibility(int module, int visibility) {
        ...
        switch (module) {
            ...
            case SCREEN_HINT:
                mOnScreenHintManager.setVisibility(visibility);
            case BEAUTY_FACE:
                mBeautyFaceManager.setVisibilitys(visibility);
                break;
            case CAMERA_SWITCH:
                ConcurrentSkipListMap<Integer, View> mQuickSwitcherList = mQuickSwitcherManager.getWeightList();
                Iterator iterator = mQuickSwitcherList.entrySet().iterator();
                int priority;
                while (iterator.hasNext()) {
                    Map.Entry map = (Map.Entry) iterator.next();
                    Integer key = (Integer) map.getKey();
                    if (key == 0) {
                        ((View) map.getValue()).setVisibility(visibility);
                    }
                }
                break;
            default:
    ...
    private class OnmPhotoManagerClickedListenerImpl implements View.OnClickListener {

        @Override
        public void onClick(View view) {
            LogHelper.e(TAG, "111111!!----mCurrentMode====" + mCurrentMode);
            if ("Photo".equals(mCurrentMode)) {
                switchFaceBeautyMode("Beauty Face");
                ((RotateImageView) mBeautyFaceManager.getView()).setImageResource(R.drawable.ic_freeme_beauty_face_selected);
            } else if ("Beauty Face".equals(mCurrentMode)) {
                switchFaceBeautyMode("Photo");
                viewOpen(IAppUi.BEAUTY_FACE);
                ((RotateImageView) mBeautyFaceManager.getView()).setImageResource(R.drawable.ic_freeme_beauty_face);
            } else if ("Video".equals(mCurrentMode)) {
                switchFaceBeautyMode("Video Beauty Face");
                ((RotateImageView) mBeautyFaceManager.getView()).setImageResource(R.drawable.ic_freeme_beauty_face_selected);
            } else if ("Video Beauty Face".equals(mCurrentMode)) {
                switchFaceBeautyMode("Video");
                ((RotateImageView) mBeautyFaceManager.getView()).setImageResource(R.drawable.ic_freeme_beauty_face);
            }
        }
    }

    private void switchFaceBeautyMode(String mode) {
        String newModeName = null;
        String newModeType = null;
        Resources resources = mApp.getActivity().getResources();
        if ("Beauty Face".equals(mode)) {
            newModeName = resources.getString(resources.getIdentifier("beautyface_dialog_title",
                    "string", mApp.getActivity().getPackageName()));
            newModeType = "Picture";
        } else if ("Photo".equals(mode)) {
            newModeName = resources.getString(resources.getIdentifier("normal_mode_title",
                    "string", mApp.getActivity().getPackageName()));
            newModeType = "Picture";
        } else if ("Video Beauty Face".equals(mode)) {
            newModeName = resources.getString(resources.getIdentifier("videobeautyface_dialog_title",
                    "string", mApp.getActivity().getPackageName()));
            newModeType = "Video";
    ...
    private void viewOff(int arg1) {
        if (!isMainThread()) {
            ...
            setUIVisibilityImmediately(arg1, View.GONE);
            LogHelper.d(TAG, "111111!! 1541+ module" + arg1);
        }
    }

    public void viewOpen(int arg1) {
        if (!isMainThread()) {
            Message msg = Message.obtain();
            msg.arg1 = arg1;
            msg.arg2 = View.VISIBLE;
            ...
            setUIVisibilityImmediately(arg1, View.VISIBLE);
        }
    }
}
