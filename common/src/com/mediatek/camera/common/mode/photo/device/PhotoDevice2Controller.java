...
class PhotoDevice2Controller extends Device2Controller implements
    ...
    public void openCamera(DeviceInfo info) {
        CameraSysTrace.onEventSystrace("photoDevice.openCamera", true);
        ...
        if (canOpenCamera(cameraId)) {
            try {
                ...
                CameraSysTrace.onEventSystrace("openCamera.doOpenCamera", true);
                doOpenCamera(sync);
                CameraSysTrace.onEventSystrace("openCamera.doOpenCamera", false);
                if (mNeedSubSectionInitSetting) {
                    CameraSysTrace.onEventSystrace("openCamera.createSettingsByStage1", true);
                    mSettingManager.createSettingsByStage(1);
                    CameraSysTrace.onEventSystrace("openCamera.createSettingsByStage1", false);
                } else {
        ...
        CameraSysTrace.onEventSystrace("photoDevice.openCamera", false);
    }
    ...
    public void takePicture(@Nonnull IDeviceController.CaptureDataCallback callback) {
        ...
        CameraSysTrace.onEventSystrace("photoDevice.takePicture", true);
        ...
        CameraSysTrace.onEventSystrace("photoDevice.takePicture", false);
    }
    ...
    public void closeCamera(boolean sync) {
        ...
        CameraSysTrace.onEventSystrace("photoDevice.closeCamera", true);
        ...
        mCurrentCameraId = null;
        CameraSysTrace.onEventSystrace("photoDevice.closeCamera", false);
    ...
    public void onPictureCallback(byte[] data,
        ...
        if (mCaptureDataCallback != null) {
            ...
            if (ThumbnailHelper.FORMAT_TAG.equalsIgnoreCase(formatTag)) {
                CameraSysTrace.onEventSystrace("photoDevice.onPostViewCallback", true);
                mCaptureDataCallback.onPostViewCallback(data);
                CameraSysTrace.onEventSystrace("photoDevice.onPostViewCallback", false);
            } else {
                CameraSysTrace.onEventSystrace("photoDevice.onJpegCallback", true);
                mCaptureDataCallback.onDataReceived(info);
                CameraSysTrace.onEventSystrace("photoDevice.onJpegCallback", false);
    ...
    private void configureSession(boolean isFromOpen) {
        ...
        CameraSysTrace.onEventSystrace("photoDevice.configureSession", true);
        try {
            if (mCamera2Proxy != null) {
                ...
                if (isFromOpen) {
                    ...
                    CameraSysTrace.onEventSystrace("configureSession.createCaptureSession.fromopen", true);
                    mCamera2Proxy.createCaptureSession(mSessionCallback,
                            mModeHandler, mBuilder, mOutputConfigs);
                    CameraSysTrace.onEventSystrace("configureSession.createCaptureSession.fromopen", false);
                    mIsPictureSizeChanged = false;
                    return;
                }
                ...
                CameraSysTrace.onEventSystrace("configureSession.createCaptureSession", true);
                mCamera2Proxy.createCaptureSession(surfaces, mSessionCallback,
                        mModeHandler, mBuilder);
                CameraSysTrace.onEventSystrace("configureSession.createCaptureSession", false);
                mIsPictureSizeChanged = false;
            }
        } catch (CameraAccessException e) {
        ...
        CameraSysTrace.onEventSystrace("photoDevice.configureSession", false);
    }
    ...
    private void repeatingPreview(boolean needConfigBuiler) {
        ...
        CameraSysTrace.onEventSystrace("photoDevice.repeatingPreview", true);
        if (mSession != null && mCamera2Proxy != null) {
            try {
                if (needConfigBuiler) {
        ...
        CameraSysTrace.onEventSystrace("photoDevice.repeatingPreview", false);
    }
    ...
    public void doCameraOpened(@Nonnull Camera2Proxy camera2proxy) {
        LogHelper.i(TAG, "[onOpened]  camera2proxy = " + camera2proxy + " preview surface = "
            ...
            CameraSysTrace.onEventSystrace("photoDevice.doCameraOpened", true);
            try {
                if (CameraState.CAMERA_OPENING == getCameraState()
                    ...
                    mFirstFrameArrived = false;
                    CameraSysTrace.onEventSystrace("donCameraOpened.onCameraOpened", true);
                    if (mModeDeviceCallback != null) {
                        mModeDeviceCallback.onCameraOpened(mCurrentCameraId);
                    }
                    CameraSysTrace.onEventSystrace("donCameraOpened.onCameraOpened", false);
                    ...
                    CameraSysTrace.onEventSystrace("donCameraOpened.setCameraCharacteristics", true);
                    mSettingDevice2Configurator.setCameraCharacteristics(mCameraCharacteristics);
                    CameraSysTrace.onEventSystrace("donCameraOpened.setCameraCharacteristics", false);
                    CameraSysTrace.onEventSystrace("donCameraOpened.updatePreviewPictureSize", true);
                    updatePreviewSize();
                    updatePictureSize();
                    CameraSysTrace.onEventSystrace("donCameraOpened.updatePreviewPictureSize", false);
            ...
            CameraSysTrace.onEventSystrace("photoDevice.doCameraOpened", false);
    }
    ...
    private final CaptureCallback mCaptureCallback = new CaptureCallback() {
        ...
        public void onCaptureProgressed(CameraCaptureSession session, CaptureRequest request,
            ...
            if (CameraUtil.isStillCaptureTemplate(request)
                ...
                CameraSysTrace.onEventSystrace("photoDevice.onP2Done", true);
                LogHelper.d(TAG, "[onCaptureProgressed] P2done comes, frame: " + num);
                updateCameraState(CameraState.CAMERA_OPENED);
                mModeDeviceCallback.onPreviewCallback(null, 0);
                CameraSysTrace.onEventSystrace("photoDevice.onP2Done", false);
            }
        }
        ...
        public void onCaptureCompleted(@Nonnull CameraCaptureSession session,
            ...
            if (CameraUtil.isStillCaptureTemplate(result)) {
                ...
                CameraSysTrace.onEventSystrace("photoDevice.onFirstFrameArrived", true);
                mFirstFrameArrived = true;
                updateCameraState(CameraState.CAMERA_OPENED);
                mModeDeviceCallback.onPreviewCallback(null, 0);
                mICameraContext.getSoundPlayback().init();
                CameraSysTrace.onEventSystrace("photoDevice.onFirstFrameArrived", false);
            }
