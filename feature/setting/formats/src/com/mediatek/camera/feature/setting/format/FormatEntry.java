...
public class FormatEntry extends FeatureEntryBase {
    ...
    public boolean isSupport(CameraApi currentCameraApi, Activity activity) {
        String heifSupport = SystemProperties.getString("ro.vendor.mtk_heif_capture_support",
                "no");
        if (CameraApi.API1.equals(currentCameraApi)) {
            return false;
        } else if (CameraApi.API2.equals(currentCameraApi) && "yes".equalsIgnoreCase(heifSupport)) {
            return true;
        } else {
            return false;
        }
