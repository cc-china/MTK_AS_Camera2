...
class BeautyFaceDevice2Controller implements
        IBeautyFaceDeviceController,
        CaptureSurface.ImageCallback,
        ISettingManager.SettingDevice2Requester {
    private static final Tag TAG = new Tag(BeautyFaceDevice2Controller.class.getSimpleName());
    ...
    //xapi add start
    private static final String BEAUTYFACELEVEL = "com.mediatek.camera2.beautyLevel";
    private CaptureRequest.Key<int[]> mBeautyFaceLevel;
    //xapi add end
     ...
     */
    BeautyFaceDevice2Controller(@Nonnull Activity activity, @Nonnull ICameraContext context) {
        LogHelper.d(TAG, "[BeautyFaceDevice2Controller]");
        mActivity = activity;
        ...
        filter.addAction(BeautyFaceDevice2Controller.class.getName());
    ...
    private void initSettings() throws CameraAccessException {
        ...
        mBeautyFaceLevel = CameraUtil.getRequestKey(
                mCameraManager.getCameraCharacteristics(mCurrentCameraId), BEAUTYFACELEVEL);
    }
    ...
    @Override
    public com.mediatek.camera.common.bgservice.CaptureSurface
    getModeSharedCaptureSurface() throws IllegalStateException {
        //not support now
    ...
    //xapi add start
    //应用层通过Camera2的session用builder键值对给hal层传值
    private BroadcastReceiver broadcastReceiver = new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {
            int[] modeAndLevel = intent.getIntArrayExtra("modeAndLevel");
            sendResult2Hal(modeAndLevel);
        }
    };

    private void sendResult2Hal(int[] modeAndLevel) {
        LogHelper.i(TAG, "[repeatingPreview] mSession =" + mSession + " mCamera =" + mCamera2Proxy);
        if (mSession != null && mCamera2Proxy != null) {
            try {
                mFirstFrameArrived = false;
                Builder builder = getSendToHalBuilder(Camera2Proxy.TEMPLATE_PREVIEW, modeAndLevel);
                mSession.capture(builder.build(), mCaptureCallback, null);
            } catch (CameraAccessException | RuntimeException e) {
                LogHelper.e(TAG, "[repeatingPreview] error");
            }
        }
    ...
    private Builder getSendToHalBuilder(int templateType, int[] modeAndLevel) throws CameraAccessException {
        LogHelper.i(TAG, "[doCreateAndConfigRequest] mCamera2Proxy =" + mCamera2Proxy);
        Builder builder = null;
        if (mCamera2Proxy != null) {
            builder = mCamera2Proxy.createCaptureRequest(templateType);
            mSettingDevice2Configurator.configCaptureRequest(builder);
            builder.addTarget(mPreviewSurface);
            builder.set(mBeautyFaceLevel, modeAndLevel);
        }
        return builder;
    }
    //xapi add end
