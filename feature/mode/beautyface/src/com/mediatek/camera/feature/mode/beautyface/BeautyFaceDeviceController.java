 ...
 */
public class BeautyFaceDeviceController implements IBeautyFaceDeviceController, SettingDeviceRequester {
    private static final Tag TAG = new Tag(BeautyFaceDeviceController.class.getSimpleName());
    ...
    private BeautyFaceHandler mRequestHandler;
     ...
     */
    BeautyFaceDeviceController(@Nonnull Activity activity, @Nonnull ICameraContext context) {
        HandlerThread handlerThread = new HandlerThread("BeautyFaceDeviceController");
        handlerThread.start();
        mRequestHandler = new BeautyFaceHandler(handlerThread.getLooper(), this);
    ...
    public void openCamera(DeviceInfo info) {
        mRequestHandler.obtainMessage(BeautyFaceDeviceAction.OPEN_CAMERA, info).sendToTarget();
    }

    @Override
    public void updatePreviewSurface(Object surfaceObject) {
        mRequestHandler.obtainMessage(BeautyFaceDeviceAction.UPDATE_PREVIEW_SURFACE, surfaceObject)
                .sendToTarget();
    }

    @Override
    public void setDeviceCallback(DeviceCallback callback) {
        mRequestHandler.obtainMessage(BeautyFaceDeviceAction.SET_PREVIEW_CALLBACK, callback)
                .sendToTarget();
    }

    @Override
    public void startPreview() {
        mRequestHandler.sendEmptyMessage(BeautyFaceDeviceAction.START_PREVIEW);
    }

    @Override
    public void stopPreview() {
        mRequestHandler.sendEmptyMessage(BeautyFaceDeviceAction.STOP_PREVIEW);
        waitDone();
    }

    @Override
    public void takePicture(@Nonnull JpegCallback callback) {
        mRequestHandler.obtainMessage(BeautyFaceDeviceAction.TAKE_PICTURE, callback).sendToTarget();
    }

    @Override
    public void updateGSensorOrientation(int orientation) {
        mRequestHandler.obtainMessage(BeautyFaceDeviceAction.UPDATE_G_SENSOR_ORIENTATION, orientation)
                .sendToTarget();
    }

    @Override
    public void closeCamera(boolean sync) {
        mRequestHandler.obtainMessage(BeautyFaceDeviceAction.CLOSE_CAMERA, sync == true ? 1 : 0)
                .sendToTarget();
        waitDone();
    }
    ...
    public Size getPreviewSize(double targetRatio) {
        ...
        mRequestHandler.obtainMessage(BeautyFaceDeviceAction.GET_PREVIEW_SIZE, values).sendToTarget();
    ...
    public void setPreviewSizeReadyCallback(PreviewSizeCallback callback) {
        mRequestHandler.obtainMessage(BeautyFaceDeviceAction.SET_PREVIEW_SIZE_READY_CALLBACK,
                callback).sendToTarget();
    }

    @Override
    public void setPictureSize(Size size) {
        mRequestHandler.obtainMessage(BeautyFaceDeviceAction.SET_PICTURE_SIZE, size).sendToTarget();
    }

    @Override
    public boolean isReadyForCapture() {
        boolean[] isReady = new boolean[1];
        mRequestHandler.obtainMessage(BeautyFaceDeviceAction.IS_READY_FOR_CAPTURE, isReady)
                .sendToTarget();
        waitDone();
        return isReady[0];
    }
    ...
    public void requestChangeSettingValue(String key) {
        mRequestHandler.removeMessages(BeautyFaceDeviceAction.REQUEST_CHANGE_SETTING_VALUE);
        mRequestHandler.obtainMessage(BeautyFaceDeviceAction.REQUEST_CHANGE_SETTING_VALUE, key)
                .sendToTarget();
    }
    ...
    public void requestChangeSettingValueJustSelf(String key) {
        ...
        mRequestHandler.removeMessages(BeautyFaceDeviceAction
                .REQUEST_CHANGE_SETTING_VALUE_JUST_SELF, key);
        mRequestHandler.obtainMessage(BeautyFaceDeviceAction.REQUEST_CHANGE_SETTING_VALUE_JUST_SELF,
                key).sendToTarget();
    }

    @Override
    public void requestChangeCommand(String key) {
        mRequestHandler.obtainMessage(BeautyFaceDeviceAction.REQUEST_CHANGE_COMMAND, key).sendToTarget();
    }

    @Override
    public void requestChangeCommandImmediately(String key) {
        mRequestHandler.obtainMessage(BeautyFaceDeviceAction.REQUEST_CHANGE_COMMAND_IMMEDIATELY, key)
                .sendToTarget();
    }

    @Override
    public void destroyDeviceController() {
        mRequestHandler.sendEmptyMessage(BeautyFaceDeviceAction.DESTROY_DEVICE_CONTROLLER);
    }
     ...
     */
    private class BeautyFaceHandler extends Handler {
         ...
         */
        public BeautyFaceHandler(Looper looper, SettingDeviceRequester requester) {
            super(looper);
        ...
        public void handleMessage(Message msg) {
            ...
            if (cancelDealMessage(msg.what)) {
                LogHelper.d(TAG, "[handleMessage] - msg = " +
                        BeautyFaceDeviceAction.stringify(msg.what) + "[dismiss]");
                return;
            }
            switch (msg.what) {
                case BeautyFaceDeviceAction.OPEN_CAMERA:
                    doOpenCamera((DeviceInfo) msg.obj);
                    break;
                case BeautyFaceDeviceAction.CLOSE_CAMERA:
                    doCloseCamera((Integer) msg.obj == 1 ? true : false);
                    break;
                case BeautyFaceDeviceAction.START_PREVIEW:
                    doStartPreview();
                    break;
                case BeautyFaceDeviceAction.STOP_PREVIEW:
                    doStopPreview();
                    break;
                case BeautyFaceDeviceAction.TAKE_PICTURE:
                    doTakePicture((JpegCallback) msg.obj);
                    break;
                case BeautyFaceDeviceAction.REQUEST_CHANGE_SETTING_VALUE:
                    ...
                    break;
                case BeautyFaceDeviceAction.REQUEST_CHANGE_SETTING_VALUE_JUST_SELF:
                    ...
                    break;
                case BeautyFaceDeviceAction.REQUEST_CHANGE_COMMAND:
                    doRequestChangeCommand((String) msg.obj);
                    break;
                case BeautyFaceDeviceAction.REQUEST_CHANGE_COMMAND_IMMEDIATELY:
                    doRequestChangeCommandImmediately((String) msg.obj);
                    break;
                case BeautyFaceDeviceAction.SET_PICTURE_SIZE:
                    doSetPictureSize((Size) msg.obj);
                    break;
                case BeautyFaceDeviceAction.SET_PREVIEW_CALLBACK:
                    mModeDeviceCallback = (DeviceCallback) msg.obj;
                    break;
                case BeautyFaceDeviceAction.SET_PREVIEW_SIZE_READY_CALLBACK:
                    mCameraOpenedCallback = (PreviewSizeCallback) msg.obj;
                    break;
                case BeautyFaceDeviceAction.GET_PREVIEW_SIZE:
                    doGetPreviewSize(msg);
                    break;
                case BeautyFaceDeviceAction.UPDATE_PREVIEW_SURFACE:
                    doUpdatePreviewSurface(msg.obj);
                    break;
                case BeautyFaceDeviceAction.UPDATE_G_SENSOR_ORIENTATION:
                    mJpegRotation = (Integer) msg.obj;
                    break;
                case BeautyFaceDeviceAction.IS_READY_FOR_CAPTURE:
                    boolean[] isReady = (boolean[]) msg.obj;
                    isReady[0] = isReadyForCapture();
                    break;
                case BeautyFaceDeviceAction.DESTROY_DEVICE_CONTROLLER:
                    doDestroyHandler();
                    break;

                case BeautyFaceDeviceAction.ON_CAMERA_OPENED:
                    doOnOpened((CameraProxy) msg.obj);
                    break;
                case BeautyFaceDeviceAction.ON_CAMERA_DISCONNECTED:
                    doOnDisconnected();
                    break;
                case BeautyFaceDeviceAction.ON_CAMERA_ERROR:
                    doOnError(msg.arg1);
                    break;
                default:
        ...
        private void doTakePicture(JpegCallback callback) {
            ...
            ((CameraActivity) mActivity).getmBeautyFaceView().setCaptureRotation(rotation);
        ...
        private boolean cancelDealMessage(int message) {
            ...
            boolean value = false;
            if (mRequestHandler.hasMessages(BeautyFaceDeviceAction.CLOSE_CAMERA)) {
                switch (message) {
                    case BeautyFaceDeviceAction.START_PREVIEW:
                    case BeautyFaceDeviceAction.STOP_PREVIEW:
                    case BeautyFaceDeviceAction.TAKE_PICTURE:
        ...
        private boolean hasDeviceStateCallback() {
            boolean value = mRequestHandler.hasMessages(BeautyFaceDeviceAction.ON_CAMERA_ERROR)
                    || mRequestHandler.hasMessages(BeautyFaceDeviceAction.ON_CAMERA_CLOSED)
                    || mRequestHandler.hasMessages(BeautyFaceDeviceAction.ON_CAMERA_DISCONNECTED)
                    || mRequestHandler.hasMessages(BeautyFaceDeviceAction.ON_CAMERA_OPENED);
        ...
        private class CameraDeviceProxyStateCallback extends CameraProxy.StateCallback {
            ...
            public void onOpened(@Nonnull CameraProxy cameraProxy) {
                ...
                synchronized (mWaitCameraOpenDone) {
                    ...
                    mRequestHandler.obtainMessage(BeautyFaceDeviceAction.ON_CAMERA_OPENED, cameraProxy)
                            .sendToTarget();
                }
            }
            ...
            public void onDisconnected(@Nonnull CameraProxy cameraProxy) {
                ...
                if (mCameraProxy != null && mCameraProxy == cameraProxy) {
                    synchronized (mWaitCameraOpenDone) {
                        mCameraState = CameraState.CAMERA_UNKNOWN;
                        mWaitCameraOpenDone.notifyAll();
                        mRequestHandler.obtainMessage(BeautyFaceDeviceAction.ON_CAMERA_DISCONNECTED,
            ...
            public void onError(@Nonnull CameraProxy cameraProxy, int error) {
                ...
                if ((mCameraProxy != null && mCameraProxy == cameraProxy)
                    ...
                    synchronized (mWaitCameraOpenDone) {
                        mCameraState = CameraState.CAMERA_UNKNOWN;
                        mWaitCameraOpenDone.notifyAll();
                        mRequestHandler.obtainMessage(BeautyFaceDeviceAction.ON_CAMERA_ERROR, error, 0,
                                cameraProxy).sendToTarget();
                    }
                }
            }
